...
// lib/queries/trainings.ts nach dem Muster von uebungen-fuer.ts, damit der
// Fachkern (lib/kern) Trainings lesen kann, ohne den Cookie-Client
// (`@/lib/supabase/server`, `next/headers`) oder `react`s `cache` mitzuziehen.
// Die Cookie-Wrapper (`getTrainingForEdit`, `getTrainingView`,
// `getTrainingPool`) bleiben in trainings.ts und rufen diese Funktionen; die
// Typen werden dort weiter exportiert, damit bestehende Importe halten.
//
// Keine Funktion hier fragt den Nutzer selbst ab — am Bearer-Client kennt
// `auth.getUser()` ohne Token keinen (lib/supabase/bearer.ts). Wer ihn
// braucht, bekommt die userId als Parameter.
...
export function mapTraining(raw: RawTraining): TrainingDetail {
...
/** Ein Training mit allem, was Editor, Ansicht und KI-Assistent zeigen —
 *  soweit die RLS es für diesen Client sichtbar macht. `null` heisst «nicht
 *  sichtbar» (nicht vorhanden, gelöscht, privat-fremd, Team ohne
 *  Mitgliedschaft — ununterscheidbar, Story #7 PC 2).
 *
 *  Mit `bearbeitbarFuer` zusätzlich eingegrenzt auf das, was diese Person
 *  bearbeiten darf: ihr eigenes Training oder eines ihrer Teams. Die
 *  SELECT-Policy lässt Team-Trainings nur bei Mitgliedern durch; der Filter
 *  grenzt die fremden öffentlichen aus, die jeder lesen darf (Editor,
 *  `getTrainingForEdit`).
 *
 *  Wirft bei einem Datenbankfehler — wie jede Query dieses Layers. */
export async function ladeTrainingDetail(
  supabase: SupabaseClient,
  id: string,
  bearbeitbarFuer?: string,
): Promise<TrainingDetail | null> {
  // Eine ungültige Kennung erreicht die uuid-Spalte nie — dort gäbe es einen
  // Datenbankfehler statt «nicht gefunden».
  if (!istUuid(id)) return null;
  let query = supabase.from("trainings").select(TRAINING_SELECT).eq("id", id);
  if (bearbeitbarFuer) query = query.or(`owner_id.eq.${bearbeitbarFuer},team_id.not.is.null`);
...
export function mapListRow(raw: RawListTraining, userId?: string): TrainingListRow {
...
export type TrainingPoolFilter = TrainingListFilters & {
  /** Nur öffentliche Trainings — auch die eigenen öffentlichen, ohne die
   *  eigenen Entwürfe. */
  nurOeffentlich?: boolean;
...
export async function getTrainingPoolFuer(
  supabase: SupabaseClient,
  userId: string | null,
  f: TrainingPoolFilter = {},
  ...
  if (f.nurOeffentlich) query = query.eq("visibility", "public");
  ...
  if (hasQuery) {
    ...
    // Ohne Suche sortiert die Datenbank — dann darf sie auch begrenzen. Mit
    // Suche sortiert erst JS nach Relevanz, die Grenze greift danach.
  ...
  const rows = (data ?? []).map((r) =>
    mapListRow(r as unknown as RawListTraining, userId ?? undefined),
  );
